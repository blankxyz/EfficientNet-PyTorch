...
def get_image_batch_features(image_paths, net, batch_size, zoom_out=1):
    ...
    for i in range(num_batches):
        inputs = make_image_tensor(image_paths[i * batch_size:(i + 1) * batch_size],
                                   zoom_out=zoom_out)
        inputs = inputs.cuda()
...
def extract_all(feature_path, base_dir, video_name2image_filenames, video_names, net,
                batch_size,
    ...
    for i in tqdm(range(len(video_names)), desc="Extracting for videos"):
        ...
        if cur_vname == 'root':
            image_paths = [os.path.join(base_dir, e) for e in
                           video_name2image_filenames[cur_vname]]
        else:
            image_paths = [os.path.join(base_dir, cur_vname, e) for e in
                           video_name2image_filenames[cur_vname]]
        try:
            import time
            start_time = time.time()
            data_features = get_image_batch_features(image_paths, net, batch_size,
                                                     zoom_out=zoom_out)
            end_time = time.time()
        ...
        if debug:
            logger.info("subdir (key name) {}, feature shape {}".format(cur_vname,
                                                                        data_features.shape))
            break
    feature_h5.close()
...
def get_image_paths(dir_path, image_filename_pattern="img_{:05d}.jpg", fps=15):
    ...
    offsets_inside_second = [3, 8,
                             13]  # the middle of every 5 frames., note this is not used for indexing.
...
# def get_image_paths_root(dir_path):

# return [os.path.join(dir_path, fpath) for fpath in os.listdir(dir_path)]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--zoom_out", type=int, default=1,
                        help="224 * zoom_out is the input spatial size")
    ...
    if s.lower() == "y":
        ...
        sub_dirs = [d for d in os.listdir(args.base_dir) if
                    os.path.isdir(os.path.join(args.base_dir, d))]
        ...
        if os.path.exists(cache_video_name2image_filenames_path):
            logger.info(
                "Loading from cache {}".format(cache_video_name2image_filenames_path))
            ...
            if len(sub_dirs) < 1:
                sub_dirs = ['root']
                video_name2image_filenames = {'root': os.listdir(args.base_dir)}
            else:
        ...
        with torch.no_grad():
            extract_all(args.feature_file, args.base_dir, video_name2image_filenames,
                        sub_dirs, extractor,
