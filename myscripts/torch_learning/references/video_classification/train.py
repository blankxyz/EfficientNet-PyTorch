...
from torchvision.datasets.samplers import DistributedSampler, UniformClipSampler, \
    RandomClipSampler
...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, data_loader, device, epoch,
                    print_freq, apex=False):
    model.train()
    metric_logger = utils.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', utils.SmoothedValue(window_size=1, fmt='{value}'))
    metric_logger.add_meter('clips/s',
                            utils.SmoothedValue(window_size=10, fmt='{value:.3f}'))
...
def _get_cache_path(filepath):
    ...
    cache_path = os.path.join("~", ".torch", "vision", "datasets", "kinetics",
                              h[:10] + ".pt")
