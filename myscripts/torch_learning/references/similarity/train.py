...
def train_epoch(model, optimizer, criterion, data_loader, device, epoch, print_freq):
    ...
    for i, data in enumerate(data_loader):
        ...
        if i % print_freq == print_freq - 1:
            ...
            print(
                '[{:d}, {:d}] | loss: {:.4f} | % avg hard triplets: {:.2f}%'.format(epoch,
                                                                                    i,
                                                                                    avg_loss,
                                                                                    avg_trip))
            running_loss = 0
...
def main(args):
    ...
    train_dataset = FashionMNIST(args.dataset_dir, train=True, transform=transform,
                                 download=True)
    test_dataset = FashionMNIST(args.dataset_dir, train=False, transform=transform,
                                download=True)
